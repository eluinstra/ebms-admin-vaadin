...
window.Vaadin.Flow['_vaadintheme_ebms-admin-vaadin_globalCss'] = window.Vaadin.Flow['_vaadintheme_ebms-admin-vaadin_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_ebms-admin-vaadin_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_ebms-admin-vaadin_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_ebms-admin-vaadin_globalCss'].push(target);
  }
  if (!document['_vaadintheme_ebms-admin-vaadin_componentCss']) {
    
    document['_vaadintheme_ebms-admin-vaadin_componentCss'] = true;
  }
